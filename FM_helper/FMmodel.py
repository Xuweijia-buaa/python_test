...
class FM(nn.Module):
    ...
    def forward(self, f_p, f_x):  # B,n.  n是每个样本的原始特征数目
        ...
        #         print(f_p.long().max())
        #         print(f_p.long().min())
        #         print(self.W.weight.shape[0])

        #        assert f_p.long().max().item() < self.W.weight.shape[0]

        # 一阶score
        w = self.W(f_p.long()).reshape(batch_size, -1)  # B,n   每个样本根据原始特征，找到对应位置处的w
        #         print(w.shape)
        #         print(f_x.shape)
        y_score1 = self.w0 + torch.sum(torch.mul(w, f_x),
                                       1)  # B,1   wixi+b  B,n -->   B,1.  要是不sum，也可以作为n个特征。之后作为deepFM的改造
